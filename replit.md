...
- **2026-02-24**: Supabase proxied through Vercel rewrites in production
  ...
  - All API calls (REST, Auth, Edge Functions) go through same-origin Vercel proxy
  - Eliminates CORS and network-level blocking issues in production
  - Dev mode still uses Vite proxy (`/supabase-proxy` for API, `/supabase-storage` for images)
  - Supabase URL and anon key hardcoded in client (public values, no env vars needed)
  - `proxyImageUrl()` utility in `src/lib/utils.ts` rewrites storage image URLs in dev mode only
